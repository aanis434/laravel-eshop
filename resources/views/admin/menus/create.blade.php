    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.menus.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('route_name') ? 'is-invalid' : '' }}" type="text" name="route_name" id="route_name" value="{{ old('route_name', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('icon_or_logo') ? 'is-invalid' : '' }}" type="text" name="icon_or_logo" id="icon_or_logo" value="{{ old('icon_or_logo', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('menu_section') ? 'is-invalid' : '' }}" name="menu_section_id" id="menu_section_id">
                    @foreach($menu_sections as $id => $menu_section)
                        <option value="{{ $id }}" {{ old('menu_section_id') == $id ? 'selected' : '' }}>{{ $menu_section }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent_menu') ? 'is-invalid' : '' }}" name="parent_menu_id" id="parent_menu_id">
                    @foreach($parent_menus as $id => $parent_menu)
                        <option value="{{ $id }}" {{ old('parent_menu_id') == $id ? 'selected' : '' }}>{{ $parent_menu }}</option>
                    @endforeach
                </select>
