...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="bangla_name">{{ trans('cruds.product.fields.bangla_name') }}</label>
                <input class="form-control {{ $errors->has('bangla_name') ? 'is-invalid' : '' }}" type="text" name="bangla_name" id="bangla_name" value="{{ old('bangla_name', $product->bangla_name) }}">
                @if($errors->has('bangla_name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('bangla_name') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.bangla_name_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="unit_id">{{ trans('cruds.product.fields.unit') }}</label>
                <select class="form-control select2 {{ $errors->has('unit') ? 'is-invalid' : '' }}" name="unit_id" id="unit_id">
                    @foreach($units as $id => $unit)
                        <option value="{{ $id }}" {{ ($product->unit ? $product->unit->id : old('unit_id')) == $id ? 'selected' : '' }}>{{ $unit }}</option>
                    @endforeach
                </select>
                @if($errors->has('unit'))
                    <div class="invalid-feedback">
