    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.delivery-types.update", [$deliveryType->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bangla_name') ? 'is-invalid' : '' }}" type="text" name="bangla_name" id="bangla_name" value="{{ old('bangla_name', $deliveryType->bangla_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $deliveryType->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\DeliveryType::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $deliveryType->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
