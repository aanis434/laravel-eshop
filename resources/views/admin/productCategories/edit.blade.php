...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-categories.update", [$productCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="parent_category_id">{{ trans('cruds.productCategory.fields.parent_category') }}</label>
                <select class="form-control select2 {{ $errors->has('parent_category') ? 'is-invalid' : '' }}" name="parent_category_id" id="parent_category_id">
                    @foreach($parent_categories as $id => $parentCateogry)
                    <option value="{{ $id }}" {{ ($productCategory->parentCateogry ? $productCategory->parentCateogry->id : old('parent_category_id')) == $id ? 'selected' : '' }}>{{ $parentCateogry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.productCategory.fields.parent_category_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required">{{ trans('cruds.productCategory.fields.status') }}</label>
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    <option value disabled {{ old('status', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\ProductCategory::STATUS_SELECT as $key => $label)
